...
void SpriteSystem::draw(entt::registry& registry, SDL_Renderer* renderer, const Camera& camera)
{
  ...
  for (auto &&[entity, sprite, position] : registry.view<SpriteComponent, PositionComponent>().each())
  {
    ...
    glm::vec3 cross_product = glm::cross(glm::vec3(0, -1, 0.0f), glm::vec3(position.orientation, 0.0f));
    ...
    SDL_RenderCopyEx(
      renderer, sprite.texture, &sprite.frame, &dst_rect, angle_deg, nullptr, sprite.flipped ? SDL_FLIP_HORIZONTAL : SDL_FLIP_NONE);
