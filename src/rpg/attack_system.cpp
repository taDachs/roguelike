...
{
  auto&& [pose, inventory, attack] =
    registry.get<PositionComponent, InventoryComponent, AttackComponent>(entity);
...
{
  for (auto&& [entity, inventory, attack, pose] :
       registry.view<InventoryComponent, AttackComponent, PositionComponent>().each())
  {
